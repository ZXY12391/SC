...
def user_tweet_process(task,proxies=None,cookies=None):
    ...
    while page_count < 5:
        ...
        try:
            ...
            # 存储json文件
            store_json(content, './filter_data/json_data/1.json')
            print('存储成功')
            time.sleep(10000)
...
def task_one_new(task):
    ...
    if flag:
        ...
        cbd['RZ_multi_public_tasks'].update_one({'twitter_url': task['twitter_url']}, {'$set':{'tweets': 1}})
...
if __name__=='__main__':
    ...
    cbd=client['Fqdl']
    user_tweet_tasks =list(cbd['RZ_multi_public_tasks'].find({'profile':1,'tweets':0}))
    user_tweet_tasks += list(cbd['RZ_multi_public_tasks'].find({'profile': 2, 'tweets': 0}))
    user_tweet_tasks += list(cbd['RZ_multi_public_tasks'].find({'profile': 3, 'tweets': 0}))
    # task_one_new(user_tweet_tasks[0])
    # cbd['RZ_multi_user_tasks'].update_one({'twitter_url': user_task['twitter_url']}, {'$set': {'profile': 1}})
    from multiprocessing.dummy import Pool
    pool = Pool(6)
    res = pool.map(task_one_new,user_tweet_tasks)