...
def task_one_new(user_task):
    try:
        ...
        if len(user_list)==0:
            ...
            return None
        if len(user_list)==1:
            cbd['RZ_multi_public_tasks'].update_one({'twitter_url': user_task.get('twitter_url')},
                                                  {'$set': {'profile': 3}})
        for x in user_list:
            ...
            cbd['RZ_multi_public_tasks'].update_one({'twitter_url':user_task.get('twitter_url')},{'$set':{'profile': 2}})
...
if __name__=='__main__':
    ...
    cbd = client['Fqdl']
    user_profile_tasks = list(cbd['RZ_multi_public_tasks'].find({'profile': 1}))
